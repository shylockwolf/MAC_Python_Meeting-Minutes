...
2. **过多片段降级**：片段数 > 20 时（如逐句时间戳），切换为按 30,000 字符分段
3. **安全拆分**：单段超过 15,000 字符时，递归拆分为子段分别处理
4. **时间范围透传**：每段携带原始时间范围，AI 根据文本相对位置估算子段落时间戳
